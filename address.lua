...
function meta.init()
    if meta._isInit then return end
    meta._isInit = true
    for _, v in ipairs(data_) do
        local k = v[1]
        local obj = {}
        obj.id = k
        obj.name = v[2]
        if string.sub(k, 3, 6) == '0000' then -- province
            obj.pid = ''
            meta._pList[#meta._pList+1] = obj
        elseif string.sub(k, 5, 6) == '00' then -- city
            obj.pid = string.sub(k, 1, 2) .. '0000'
            meta.addChild(obj)
        else
            obj.pid = string.sub(k, 1, 4) .. '00' -- county
            meta.addChild(obj)
        end
        meta._dict[k] = obj
    end
...
function meta.addChild(obj)
    local pid = obj.pid
    local pObj = meta._dict[pid]
    if pObj ~= nil then
        pObj.children = pObj.children or {}
        pObj.children[#pObj.children + 1] = obj
    end
end
